...
$slashesTable = rex::getTable(Cke5DatabaseHandler::CKE5_SLASHES);
$message = '';

if ($func === 'delete') {
    $message = Cke5ListHelper::deleteData($slashesTable, $id);
    rex_extension::registerPoint(new rex_extension_point('CKE5_PROFILE_SLASHES_DELETE', $id));
    $func = '';
}

if ($func === '') {
    // instance list
    $list = rex_list::factory("SELECT id, icon, title, description FROM $slashesTable ORDER BY id");
    ...
    // action add/edit
    $thIcon = '<a href="' . $list->getUrl(['func' => 'add']) . '" title="' . rex_i18n::msg('cke5_add_slashes') . '"><i class="rex-icon rex-icon-add-action"></i></a>';
    ...
    // name
    $list->setColumnLabel('icon', rex_i18n::msg('cke5_icon_slashes'));
    $list->setColumnParams('icon', ['func' => 'edit', 'id' => '###id###', 'start' => $start]);

    // element
    $list->setColumnLabel('title', rex_i18n::msg('cke5_title_slashes'));
    $list->setColumnParams('title', ['func' => 'edit', 'id' => '###id###', 'start' => $start]);

    // description
    $list->setColumnLabel('description', rex_i18n::msg('cke5_slashes_description'));
    ...
    $fragment->setVar('title', rex_i18n::msg('cke5_list_slashes'));
    ...
    $form = rex_form::factory($slashesTable, '', 'id=' . $id, 'post');
    ...
    if ($func === 'edit') {
        ...
        if (!is_array($result)) {
            ...
            $prefix = rex::getTable('cke5_slashes') . '.';
        }
    }
    ...
    $field->setAttribute('placeholder', rex_i18n::msg('cke5_title_slashes_placeholder'));
    $field->getValidator()->add('notEmpty', rex_i18n::msg('cke5_title_slashes_empty_error'));

    // description
    $field = $form->addTextField('description');
    $field->setLabel(rex_i18n::msg('cke5_description_style'));
    $field->setAttribute('placeholder', rex_i18n::msg('cke5_description_slashes_placeholder'));

    // icon
    $field = $form->addTextField('icon');
    $field->setLabel(rex_i18n::msg('cke5_icon_slashes'));
    $field->setAttribute('placeholder', rex_i18n::msg('cke5_icon_slashes_placeholder'));
...
//    $field->getValidator()->add('notEmpty', rex_i18n::msg('cke5_data_area_empty_error'));
    ...
    $fragment->setVar('title', ($func === 'edit') ? rex_i18n::msg('cke5_slashes_edit') : rex_i18n::msg('cke5_slashes_add'));
